/**
 * Restart durability test for PGlite file-backed lane
 *
 * This script tests that jobs survive a hard process kill and are
 * resumed/completed in a fresh process pointing at the same file-backed store.
 *
 * Usage:
 *   bun run restart/restart-pglite-file.ts
 *
 * Flow:
 *   1. Clean up previous test data
 *   2. Start process 1: Enqueue jobs and hold process open
 *   3. Hard-kill process 1 while jobs are still processing
 *   4. Start process 2: Open same file-backed store and verify jobs were resumed
 *   5. Process 2: Wait for jobs to complete
...
const DB_DIR = ".conformance-test-data-restart";
const MARKER_FILE = ".restart-marker.json";

const sleep = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

interface RestartTestResult {
  phase: string;
  success: boolean;
  message: string;
}

const results: RestartTestResult[] = [];
...
async function phase1EnqueueAndKill() {
  ...
  const boss = new PgBoss({ db, newJobCheckInterval: 100 });
  ...
  try {
    ...
    // Simulate hard kill: exit immediately (mimics SIGKILL)
...
async function main() {
  ...
  console.log("RESTART DURABILITY TEST - PGlite File-Backed\n");
  ...
  if (phase === "phase1") {
    ...
    const phase1Process = spawn("bun", ["run", "restart/restart-pglite-file.ts", "phase1"], {
      cwd: process.cwd(),
      stdio: "inherit",
    });
    ...
    const phase2Process = spawn("bun", ["run", "restart/restart-pglite-file.ts", "phase2"], {
      cwd: process.cwd(),
      stdio: "inherit",
    });
