...
| **3 (COMPLETE)** | **Events: envelope + HLC + outbox + bus** | Emas-pos has `@aurum/events` (canonical envelope struct, Lamport-clock timestamps for ordering, transactional outbox pattern, in-proc bus for subscribers). Used for: receipt posted event → triggers PDF, sale event → ledger postings, sync heartbeat events to control plane. | **`modules/events` ✓ SHIPPED** (Wave E, 2026-07-16): envelope (zod) + HLC + upcast + transactional outbox + bus; 47 tests | None — pull via `bun scripts/add.ts events --into <emas-pos>` | **COMPLETE** | Exactly-once EFFECT proven: per-event marking (poison event cannot wedge the stream), crash recovery via cross-process SIGKILL on file-backed PGlite, payload round-trip asserted. Single-drainer per (org,branch) assumption documented in module README. |
...
| **5 (COMPLETE)** | **Agentic: registry.execute() + MCP tools** | Emas-pos has `@aurum/registry` — single audited door for all service calls (REST + MCP + future agents). Zod contract → tool schema derivation (no hand-written JSON Schema). | **`modules/agentic` ✓ SHIPPED** (Wave E, 2026-07-16): `defineTool` + `createRegistry().execute()` + `createMcpHandler`; 15 tests | None — pull via `bun scripts/add.ts agentic --into <emas-pos>` | **COMPLETE** | Autonomy gate proven: outbound tools return drafts unless caller ctx carries `auto` (default `suggest`); ALL outcomes audited at the door incl. denials; `tools/list` filtered by `can()`; JSON schemas derived from zod with `.describe()` propagation. Framework-free `(Request)=>Response` MCP transport. |
...
- [x] **Events module**: DONE (Wave E, 2026-07-16) — `modules/events`: envelope + HLC + upcast + outbox + bus, de-domained, crash-recovery proven on file-backed PGlite. Pull: `bun scripts/add.ts events --into <emas-pos>`.
- [x] **Agentic module**: DONE (Wave E, 2026-07-16) — `modules/agentic`: `registry.execute()` + zod-derived tool schemas + autonomy gate + MCP JSON-RPC transport, de-domained. Pull: `bun scripts/add.ts agentic --into <emas-pos>`.
