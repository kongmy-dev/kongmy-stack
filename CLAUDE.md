...
## Open — pending design session (do NOT improvise)

- **AuthZ model**: role enum vs permission strings, requirement declared on the contract/route definition (so OpenAPI + MCP tools + autonomy gate consume one declaration), impersonation interplay. Until decided, the worked example uses a placeholder `requireRole('admin')` middleware clearly marked `// AUTHZ-PENDING`.
