...
> Decided 2026-07-13. Full rationale in internal notes. Working summary in `CLAUDE.md`. This file: what to build, in what order, and why each piece earns its place.
...
**Construction with live consumers, not extraction.** Consumer products consume this template as they form. Build order = whatever consumers need next. This keeps every abstraction honest — nothing enters the template that a real product didn't pull — and avoids the historical failure mode where platform code is born inside a product and never extracted.
...
## Phase 1 (both halves, consumer-driven)

### 1a. Spikes first (half-day each — they de-risk locked-but-unverified choices)
- **Spike A: pg-boss on PGlite.** pg-boss was chosen for battle-tested retry/backoff/poison handling, but consumers may run embedded PGlite. If pg-boss runs on PGlite → the minimal-SQL fallback impl dies (less code). If not → fallback is confirmed as the PGlite lane. Either result is a win; not knowing is the only failure.
...
- UI vendored from the sapphire-ui registry (~15 components + admin blocks)
- `features/` structure with one worked example feature (CRUD + list + form) as the copyable pattern

### 1d. Consumer consumption
- Consumers scaffold from the skeleton; their parallel theme + UI packages are retired; their platform needs (money, events) get built HERE as modules and vendored back.

## Phase 2+ (pull-driven, in likely order)

1. `modules/money` — Money/Weight VOs (from private consumer references, generalized), fast-check property tests. Consumers pull as needed.
2. `modules/queue` — the interface + pg-boss impl (+ PGlite impl per Spike A) + scheduler.
3. `modules/events` — canonical envelope + HLC + transactional outbox + in-proc bus (from private consumer references, generalized).
...
5. `modules/connector` — canonical model + real/fake gateway adapter pair + sync-job tables + verify-invariant scripts (from private middleware reference, generalized).
...
- [x] Consumer products consuming the skeleton for real screens end-to-end — *scaffold signal issued; tracked in consumer repos*
...
- No Next.js, no Turbo, no message broker. Escalation paths are documented in internal notes; they trigger on evidence, not anticipation.
